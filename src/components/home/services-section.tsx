...
const serviceImages: Record<string, string> = {
  "Личные границы": "/service-lichnye-granitsy-v2.png",
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((service, index) => {
            ...
            const isBoundariesCard = imageSrc?.startsWith(
              "/service-lichnye-granitsy-v2.png",
            );
              ...
              <FadeIn key={`${service.title}-${index}`} delay={index * 100}>
                <div className="group relative aspect-[2/3] h-full overflow-hidden rounded-2xl border border-sage-light/15 bg-white transition-all duration-300 hover:-translate-y-1 hover:border-sage/30 hover:shadow-xl">
                  {imageSrc ? (
                    <div
                      className={`absolute inset-0 ${
                        isBoundariesCard
                          ? "bg-gradient-to-b from-sage-light/20 via-white to-white"
                          : "bg-sage-light/10"
                      }`}
                    >
                      <Image
                        ...
                        className={`transition-transform duration-500 group-hover:scale-105 ${
                          isBoundariesCard
                            ? "object-contain object-center px-1 pt-1 pb-0 opacity-100 contrast-105 scale-[1.12]"
                  ...
                  <div
                    className={`absolute inset-0 ${
                      isBoundariesCard
                        ? "bg-gradient-to-t from-forest/68 via-forest/10 to-transparent"
                        : "bg-gradient-to-t from-forest/74 via-forest/20 to-transparent"
                    }`}
                  />
                  <div className="absolute inset-x-0 bottom-0 rounded-t-[1.5rem] rounded-b-2xl border border-white/10 bg-white/[0.02] p-5 shadow-lg backdrop-blur-md sm:p-6">
